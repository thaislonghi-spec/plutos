...
def copart_tabela_xlsx(s: dict, comp: str, nomes: dict) -> io.BytesIO:
    """Coparticipação de frete do Full por SKU, nas três tabelas do canal.
    Aba 1 = ORION (o que entra no preço) · Aba 2 = impacto da troca de tabela."""
    wb = Workbook()
    wb.remove(wb.active)

    cab = ["SKU", "Descrição", "Coparticipação VIGENTE por unidade (R$)", "Tabela vigente",
           "Unidades vendidas no Full", "Coparticipação do mês (R$)"]
    ls = [[m["sku"], m.get("descricao") or "", m["vigente_un"], nomes.get(s["vigente"], s["vigente"]),
           m["unidades"], m["vigente_mes"]] for m in s["linhas"]]
    _aba(wb, "ORION_copart_por_unidade", cab, ls, [16, 46, 30, 20, 22, 24], moeda=(3, 6))

    cab2 = ["SKU", "Descrição", "Unidades", "Antiga / un", "Nova −40% / un", "Nova / un",
            "Sobe por unidade (nova − antiga)", "Antiga no mês", "Nova −40% no mês", "Nova no mês",
            "Impacto no mês (nova − antiga)"]
    ls2 = [[m["sku"], m.get("descricao") or "", m["unidades"], m["antiga"], m["nova_40"], m["nova"],
            round(m["nova"] - m["antiga"], 2), m["antiga_mes"], m["nova_40_mes"], m["nova_mes"],
            round(m["nova_mes"] - m["antiga_mes"], 2)]
           for m in sorted(s["linhas"], key=lambda x: -(x["nova_mes"] - x["antiga_mes"]))]
    _aba(wb, "Impacto_da_troca_de_tabela", cab2, ls2, [16, 44, 12, 13, 16, 12, 28, 16, 18, 15, 26],
         moeda=(4, 5, 6, 7, 8, 9, 10, 11))

    if s.get("sem_tabela"):
        _aba(wb, "Vendidos_sem_tabela", ["SKU", "Descrição", "Unidades"],
             [[m["sku"], m.get("descricao") or "", m["unidades"]] for m in s["sem_tabela"]], [16, 46, 12])

    ws = wb.create_sheet("Como_ler", 0)
    for l in [
        [f"PLUTOS · Coparticipação do Full por SKU · {comp}"],
        [],
        ["Tabela vigente:", nomes.get(s["vigente"], s["vigente"])],
        ["SKUs vendidos no Full:", s["skus"], "unidades:", s["unidades"]],
        [],
        ["", "Por unidade", "No mês"],
        ["Tabela antiga", s["un_media"]["antiga"], s["por_tabela"]["antiga"]],
        ["Tabela nova −40%", s["un_media"]["nova_40"], s["por_tabela"]["nova_40"]],
        ["Tabela nova (cheia)", s["un_media"]["nova"], s["por_tabela"]["nova"]],
        [],
        ["Alta da nova −40% sobre a antiga", f"{s['alta_40']:.0f}%", s["delta_40"]],
        ["Alta da nova cheia sobre a antiga", f"{s['alta_nova']:.0f}%", s["delta_nova"]],
        [],
        ["O valor por SKU vem da tabela do canal; as unidades são as vendidas no Full na competência."],
        ["A coparticipação é o pedaço do frete que a Multimóveis paga — entra no custo do produto, não é rebate."],
    ]:
        ws.append(l)
    ...
    for r in (7, 8, 9, 11, 12):
        for c in ("B", "C"):
            if isinstance(ws[f"{c}{r}"].value, (int, float)):
                ws[f"{c}{r}"].number_format = BRL
...
def entregas_xlsx(e: dict, comp: str, nomes_tab: dict) -> io.BytesIO:
    """ENTREGAS BY PLUTOS — o arquivo ÚNICO que sobe no ORION.
    Aba 1: uma linha por SKU × canal, com o custo de entrega por unidade.
    Aba 2: pivô SKU × canal, para comparar canal a canal no mesmo SKU."""
    wb = Workbook()
    wb.remove(wb.active)

    cab = ["SKU", "Descrição", "Canal", "Modalidade", "CUSTO DE ENTREGA POR UNIDADE (R$)",
           "Logística por unidade (R$)", "Frete por unidade (R$)", "Fonte do frete",
           "Unidades no período", "Confiança", "Peso (kg)", "Cubagem (m³)",
           "R$/un manuseio", "Coleta por unidade (tabela)", "De onde vem"]
    ls = [[l["sku"], l.get("descricao") or "", l["canal"], l["modalidade"], l["custo_un"],
           l["logistica_un"], l["frete_un"], l["fonte_frete"], l.get("unidades"),
           l.get("confianca"), l.get("peso"), l.get("cubagem"), l.get("manuseio_un"),
           l.get("coleta_un"), l["fonte"]]
          for l in e["linhas"]]
    _aba(wb, "ORION_entregas_por_sku", cab, ls,
         [16, 44, 18, 14, 30, 24, 22, 26, 18, 14, 11, 12, 16, 24, 30],
         moeda=(5, 6, 7, 13, 14))

    cols = e["colunas"]

    # ---- COMPARATIVO: só os SKUs que vendem em MAIS DE UM canal ------------
    # É a aba que mostra quanto a MESMA peça custa para entregar em cada canal.
    cabc = (["SKU", "Descrição"]
            + [f"{c} — custo/un" for c in cols]
            + [f"{c} — unidades" for c in cols]
            + ["Canal mais barato", "Canal mais caro", "Menor (R$)", "Maior (R$)",
               "DIFERENÇA (R$)", "Diferença (%)", "Conferir?"])
    lsc = []
    for p in e.get("comparativo") or []:
        pouca = any(p["confianca"].get(c) != "ok" for c in p["canais"])
        lsc.append([p["sku"], p.get("descricao") or ""]
                   + [p["canais"].get(c) for c in cols]
                   + [p["unidades"].get(c) for c in cols]
                   + [p["mais_barato"], p["mais_caro"], p["menor"], p["maior"],
                      p["spread"], p["spread_pct"],
                      "sim — pouca base num dos canais" if pouca else ""])
    _aba(wb, "Comparativo_mesmo_SKU", cabc, lsc,
         [16, 42] + [20] * len(cols) + [16] * len(cols) + [24, 24, 14, 14, 18, 16, 30],
         moeda=tuple(range(3, 3 + len(cols))) + tuple(range(3 + 2 * len(cols) + 2, 3 + 2 * len(cols) + 6)))

    cab2 = ["SKU", "Descrição"] + cols + ["Menor", "Maior", "Diferença entre canais", "Em quantos canais"]
    ls2 = [[p["sku"], p.get("descricao") or ""] + [p["canais"].get(c) for c in cols]
           + [p["menor"], p["maior"], p["spread"], p["n_canais"]] for p in e["pivo"]]
    _aba(wb, "Pivo_sku_x_canal", cab2, ls2,
         [16, 44] + [22] * len(cols) + [12, 12, 24, 18],
         moeda=tuple(range(3, 3 + len(cols) + 3)))

    ws = wb.create_sheet("Como_ler", 0)
    for l in [
        [f"PLUTOS · ENTREGAS por SKU · {comp}"],
        [],
        ["O que é este arquivo"],
        ["", "O custo de ENTREGA por unidade, SKU a SKU, em cada canal. É o único arquivo de entregas"],
        ["", "que o ORION precisa: já vem com a conta fechada e dizendo de onde veio cada pedaço."],
        [],
        ["SKUs", e["skus"], "Canais/modalidades", e["canais"], "Custo médio por unidade", e["media"]],
        ["SKUs que vendem em mais de um canal", e.get("comparativo_n", 0),
         "Soma das diferenças por unidade", e.get("comparativo_rs", 0)],
        ["", "A aba Comparativo_mesmo_SKU mostra a MESMA peça lado a lado em cada canal."],
        ["", "Entrega mais cara num canal tem de aparecer no preço DAQUELE canal, não numa média."],
        [],
        ["Como cada canal é calculado"],
        ["Magazine Luiza · Full", "logística (manuseio + armazenagem + tempo de estoque + coleta rateada) do que o canal COBROU no mês"],
        ["", f"+ coparticipação de frete da TABELA vigente ({nomes_tab.get(e['vigente'], e['vigente'])})"],
        ["", "Precificação usa TABELA, não o realizado: é o valor que será cobrado na próxima venda."],
        ["Mercado Livre · Coletas", "custo do Coletas do último pedido válido daquele anúncio"],
        [],
        ["Atenção"],
        ["", "CONFIANÇA: 'pouca base' = o custo por unidade saiu de menos de 5 unidades no período."],
        ["", "Costuma ser item parado, que pagou armazenagem o mês inteiro e vendeu uma peça — o custo"],
        ["", "unitário fica inflado e NÃO deve ser usado para precificar sem olhar. Use o da tabela ou o"],
        ["", "de um mês com giro."],
        ["", "Rebate NÃO entra aqui. Este arquivo é CUSTO de entrega; o rebate sai no EXPORT REBATES."],
        ["", "Quando a tabela de coparticipação vigente mudar em Parâmetros, este arquivo muda junto."],
    ]:
        ws.append(l)
