...
def resumo(linhas: list[dict], cancelados: int = 0) -> dict:
    ...
    for l in linhas:
        ...
        d["pedidos"] += 1; d["venda"] += l["valor_prod"]; d["comissao"] += arred.sis_exato(l, False) - (l["tarifa"] or 0.0)
        d["cupom"] += l["rebate_rs"]; d["total"] += (l.get("rebate_rs") or 0.0) + (arred.sis_exato(l, False) - (l["tarifa"] or 0.0)) + (l.get("rebate_frete") or 0.0)
    ...
    return {
        ...
        "rebate_rs": s("rebate_rs"), "rebate_comissao": reb_com, "rebate_frete": 0.0, "rebate_total": reb_tot,
