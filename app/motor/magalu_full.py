...
def ler(caminho: str) -> tuple[str, list[dict], dict]:
    ...
    for n, l in enumerate(it, 1):
        ...
        if qual == "copart":
            ...
            dc = _data(_pega(cab, l, "data cobranca"))
            out.append({"tipo": qual, "cd": "", "sku": sku, "produto": "",
                        "data": str(d) if d else "", "agenda": "", "espaco": 0.0,
                        # a competência da coparticipação é o MÊS DO PEDIDO — a
                        # "data cobrança" do Magalu é da política/coleta e volta
                        # meses atrás (o ciclo é de 20 a 20, não fecha no mês)
                        "comp_pedido": (f"{d.year}-{d.month:02d}" if d else ""),
                        "data_cobranca": str(dc) if dc else "",
    ...
    if qual == "copart":
        ...
        cobr: dict[str, float] = {}
        for x in out:
            comps[x["comp_pedido"] or "?"] = round(comps.get(x["comp_pedido"] or "?", 0.0) + x["valor"], 2)
            m = (x["data_cobranca"] or "?")[:7]
            cobr[m] = round(cobr.get(m, 0.0) + x["valor"], 2)
        diag["por_competencia"] = dict(sorted(comps.items()))
        diag["por_cobranca"] = dict(sorted(cobr.items()))
        diag["pedidos"] = len({x["pedido"] for x in out})
    return qual, out, diag


def resumo(cobrancas: list[dict], copart_por_sku: dict | None = None, nomes: dict | None = None,
           vendidos: dict | None = None, cubagens: dict | None = None, comp_ref: str = "",
           de: str = "", ate: str = "") -> dict:
    ...
    fora_comp = 0.0
    for c in cb:
        if c["tipo"] != "copart" or not c["sku"]:
            continue
        # só a coparticipação dos PEDIDOS desta competência entra no custo dela
        if comp_ref and c.get("comp_pedido") and c["comp_pedido"] != comp_ref:
            fora_comp = round(fora_comp + c["valor"], 2)
            continue
    ...
    return {
        ...
        "copart_medida": bool(medida), "copart_fora_comp": fora_comp,
